...
#
# USAGE: ./restrict2commonsDeLuxe.pl inputFile_1 .... inputFile_k > outputFile
#
# SPECIFS INTERNES : 
...
#   @arb, @fils , @pere , @tax;
   my %taxThisTree = ();
    ...
    $chaine = $arb[$numTree] . "100:100"; # ajoute un support et une lgr fictive au dessus racine 
...
sub rmvTaxFromTree { # !!! has to be done while preserving order or remaining nodes (for calling scripts purposes)
   ...
   foreach $ndTax (@NdsTax) {
      ...
      if ($#frer>1) { # case where rmving node is enough : no branch merge, no replacing.
         ...
         # adding lengths of the branches to be merged
...
#    $fic = shift @ARGV;
    ...
    for ($a=0;$a<=$#AllTrees;$a++) {
       ...
       # If tree has no branch lengths, add fake one temporarily
       if ($AllTrees[$a]=~ /\:/) { $FakeLgr[$inTree]=0; CheckAllLgth $AllTrees[$a] ;}       
       else { verbose "add fake lengths"; 
            $FakeLgr[$inTree]=1; $AllTrees[$a]=LgrBidon($AllTrees[$a]) ; }
       # If tree has no branch support, 
       if     ($AllTrees[$a]=~ /\)\d/) {verbose "add fake supports temporarily";
       $FakeSupport[$inTree]=0; CheckAllSupport $AllTrees[$a] ;}  
       else { $FakeSupport[$inTree]=1;
            $AllTrees[$a]=SuppBidon($AllTrees[$a]) ; }
      $inTree++;
    }
